...
public class SBWEB235VerifyLinksOnRecoveredTissuePage extends Script {
	private static final Logger logger = Logger.getLogger(SBWEB235VerifyLinksOnRecoveredTissuePage.class);
...
*/
		...
		public boolean addTissue(String RecoveryId, String txtUser,String strCollectionAgent, String dteDate, String strHospitalCode)
		{
			...
			Page("pgeNewRecoveredTissue").Element("ftrHospital").Type(strHospitalCode);
			waitForElementToRefresh(1);
        ...
        //''@     Failure - False
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@Example: blnStatus= verifySearch(PageName, ElementName, "WC14F036")        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@Created by[Date]: Ashish Khare[11-Jul-2014]
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@Reviewed by[Date]: 
        //''@---------------------------------------------------------------------------------------------------------------------------
        ...
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@###########################################################################################################################			
		public boolean verifySearch(String Page, String Element, String text) throws InterruptedException 
		{
			boolean blnFlag = false;
			...
			String eleLink = Page("pgeRecovered_Tissue").Element("lnkRecoveryId").GetText();
			...
			{
				blnFlag = false;
				Err.Description = "Searched link not displayed in grid.";
			}
			return blnFlag;
		}
		...
		public boolean clickLoadReviewForm(String strFilePath) 
		{
			...
			{
				blnFlag=Page("pgeRecovered_Tissue").Element("lnkLoad_Form").Click(200);
				if (blnFlag==true)
				{
					...
					if (blnFlag==true)
					{
						...
						Page("pgeRecovered_Tissue").Element("lnkLoad_Form").Click(200);
					}
					else		
...
//			boolean bool=Page("pgeRecovered_Tissue").Element("btnLoad_File").Click(200);
			if (blnFlag==true)
			{
				blnFlag=true;
			}
        ...
        //''@Objective: This Function verifies uploaded 'pdf' file and pass 'pdf' file 'Recovered Tissue Form Files' page.
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@Return Desc: 
        //''@     Success - True
        //''@     Failure - False
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@Example: blnStatus= verifyScan()        //''@---------------------------------------------------------------------------------------------------------------------------
        ...
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@###########################################################################################################################		
		public boolean verifyScan() 
		{
			boolean blnFlag = false;
			driver.findElement(By.xpath("//a[contains(text(),'Verify correct scan')]")).click();
			driver.findElement(By.xpath("//input[contains(@id,'pgsbcbEditWindow')]")).click();
			driver.findElement(	By.xpath("//textarea[@id='dialogform:pgEditWindowFollowup']")).sendKeys("Ok");
			driver.findElement(By.xpath("//button[@id='dialogform:cmdSaveEditWindow']")).click();
			String button = driver.findElement(By.xpath("//button[@id='centerform:cmdpass']")).getText();
			if (button.equalsIgnoreCase("Pass")) 
			{
				blnFlag = true;
				driver.findElement(By.xpath("//button[@id='centerform:cmdpass']")).click();
			} 
			else 
			{
				blnFlag = false;
