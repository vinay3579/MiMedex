...
public class SBWEB388VerifyOrdersInRejectedStateFunctionality extends Script {
	private static final Logger logger = Logger.getLogger(SBWEB388VerifyOrdersInRejectedStateFunctionality.class);
...
/*  	// ########################################################################################################
		...
		blnFlag=selectAndMoveTissue(strTissueId);
		if (blnFlag == true) 
		{
		...
		// ########################################################################################################
		// Step 12 - Select 'Order Replacement' check box on 'Consignment Purchase Order' page.
		// ########################################################################################################
		strStepDesc = "Select 'Order Replacement' check box on 'Consignment Purchase Order' page.";
		strExpResult = "'Order Replacement' check box should be selected on 'Consignment Purchase Order' page.";
		blnFlag=Page("pgeConsignmentPurchaseOrder").Element("objOrderReplacement").Click(20);
		if (blnFlag == true) 
		{
			reporter.ReportStep(strStepDesc, strExpResult, "'Order Replacement' check box is selected on 'Consignment Purchase Order' page.", "Pass");
		}
		else 
		{
			reporter.ReportStep(strStepDesc, strExpResult, Err.Description, "Fail");
		...
		// ########################################################################################################
		// Step 19 - Click on 'Rejected' button and verify opened window.
		// ########################################################################################################
		...
		// ########################################################################################################
		// Step 20 - Fill rejection details and click 'Commit' button in 'Reject Purchase Order' window.
		// ########################################################################################################
		...
		// ########################################################################################################
		// Step 21 - Click on 'Return' button and verify navigated page.
		// ########################################################################################################
		...
		// ########################################################################################################
		// Step 22 - Click 'Rejected' tab on Consignment Billing page.
		// ########################################################################################################
		...
		//########################################################################################################
		//Step 23 - Search same purchase order under rejected tab on 'Consignment Billing' page.
		//########################################################################################################
...
*/
		...
		public boolean verifySearch(String Page, String Element, String newElement, String text) 
		{
			...
			{
				Err.Description="Timed out after 100 seconds.Searched element not found or search functinality not working.";
			}
			if( blnFlag==true)
			{			
		...
		public void waitForPageRefresh() 
		{
			...
			{
				Err.Description="Timed out after 20 seconds wait.";
			}
		}
...
//				Process process = new ProcessBuilder("./AutoIt/Upload.exe",strFilePath, strDialog).start();
        ...
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@###########################################################################################################################			
		public boolean selectAndMoveTissue(String strTissueId) 
		{
			boolean blnFlag = false;
			...
			if (strSourceTissueId != null && !strSourceTissueId.isEmpty())
			{
				...
				Err.Description="Tissues '"+strTissueId+"' not displayed under 'Available Tissues' grid hence unable to move to 'Selected Tissues' grid.";
