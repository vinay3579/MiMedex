...
public class SBWEB236VerifyAddRecoverTissueFunctionality extends Script {
	private static final Logger logger = Logger.getLogger(SBWEB236VerifyAddRecoverTissueFunctionality.class);
    ...
    public void run() throws Exception {
		...
		blnFlag = addTissue(strUserName, strPassword, strCollectionAgent, dteDate, strSurgeonName, strFirstName, strLastName);
		...
		//########################################################################################################
		//Step 10 - Verify QA Transition status on "Recovered Tissue" page. 
		//########################################################################################################
		strStepDesc = "Verify 'QA Transition' status on 'Recovered Tissue' page.";
		strExpResult = "'QA Transition' status should be displayed for added Recovery Id '"+strRecoveryId+"'.";
		blnFlag=verifyTissueStatus("pgeRecovered_Tissue", "objStatus", "QA Transition");
		if (blnFlag == true)
		{
			reporter.ReportStep(strStepDesc, strExpResult, "'QA Transition' status displayed for added Recovery Id '"+strRecoveryId+"'.", "Pass");
		}
		else
		{
			reporter.ReportStep(strStepDesc, strExpResult, Err.Description, "Fail");
