...
public class SBWEB347VerifyPrintPackagingFormDonorHistory extends Script {
	private static final Logger logger = Logger.getLogger(SBWEB347VerifyPrintPackagingFormDonorHistory.class);
...
//			String xTargetTissue="//ul[contains(@class,'ui-picklist-target')]/li[@data-item-label='"+strTissueId+"']";
		...
		//''@###########################################################################################################################
        //''@Function ID: 
        //''@Function Name: VerifyAddReturnGoodsAuthErrorMessage
        //''@Objective: This Function selects a customer from the Customer list, input an invalid/junk value in specified field and verify
		...
		//''@---------------------------------------------------------------------------------------------------------------------------
		//''@Input Parameters: 
        //''@	strCustomer - customer to be selected from customer list
		//''@ 	strSearchText - Invalid/Junk values to be entered
		//''@   strExpText - expected text to be validated
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@Example: blnStatus = VerifyRecoveredTissueProcessScanMessage(strRefrigerator,strRecoveryId);
		//''@---------------------------------------------------------------------------------------------------------------------------
        //''@Created by[Date]: Kamlesh Yadav[04-24-2015]
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@Reviewed by[Date]: 
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@History Notes: 
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@###########################################################################################################################			
		public boolean VerifyAddReturnGoodsAuthErrorMessage(String strCustomer,String strSearchText,String strExpText) 
		{
			boolean blnFlag = false;	
			...
			try
			{
				...
				if (blnFlag)
				{
					blnFlag = clearAllFieldAndVerifyErrorMessage("pgeReturnedGoods","btnSave","pgeCommon","objUIMessageErrorSummary",strExpText,"pgeReturnedGoods", "txtRestockingFee",strSearchText,"txtDate");
					if (!blnFlag)
					{
						Err.Description="Failed to verify error message '"+strExpText+"'.";	
					}
				}
				else
					Err.Description="Failed to select customer '"+strCustomer+"' from customer list.";		
			}
        ...
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@###########################################################################################################################		
		public boolean fillEntriesForAllFieldAddNewUser(String strLoginName, String strFullName,String  strEmail, String strConfMessage) 
		{
			boolean blnFlag = false;
			try
			{
