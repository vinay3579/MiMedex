...
public class SBWEB461VerifySearchFunctionsNegativeFunctionality extends Script {
	private static final Logger logger = Logger.getLogger(SBWEB461VerifySearchFunctionsNegativeFunctionality.class);
...
/* 		//########################################################################################################
		//Step 5 - 	Verify that if search functionality working properly in Lab Test Compliance grid.
		//########################################################################################################
		strStepDesc = "Verify search functionality working properly in 'Lab Test Compliance' grid.";
		strExpResult = "Searched result should be displayed in the 'Lab Test Compliance' grid.";
		blnFlag=verifySearch("pgeLabTestCompliance","txtTestName","lnkTestName", strTestName);
		if (blnFlag == true)
		{
			reporter.ReportStep(strStepDesc, strExpResult, "Search functionality working properly in Lab Test Compliance grid in the Test Name field with value '"+strTestName+"'.", "Pass");
		}
		else
		{
			reporter.ReportStep(strStepDesc, strExpResult, Err.Description, "Fail");
...
/* 				String tmpXpath= Page(Page).Element(Element).GetXpath();
				...
				Page("pgeCommon").Element("objPFModuleLabel").Click(20);
				final String tempSource = driver.getPageSource();
		...
		//''@---------------------------------------------------------------------------------------------------------------------------
		//''@Input Parameters: 
		//''@	pgeName - Page which contains the search box.
		//''@	strSearchBox - Name of the column to search value.
		//''@   strSearchText - Text to be typed in the search input field.
		//''@	pgeCommon - Page which contains the element objNoRecordsFound.
		//''@	eleExpectedObject - Object name expected after search to get the runtime text to validate with expected text.
		//''@	strExpectedText - Expected text of the expected object after search.
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@Example: blnStatus = verifySearchNoResults("pgeFieldTracking", "Display Tab", "Search String", "objNoRecordsFound", "No records found");     
        ...
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@###########################################################################################################################			
		public boolean verifySearchResults(String pgeName, String strSearchBox, String strSearchText, String pgeCommon, String eleExpectedObject, String strExpectedText)
		{
			boolean blnFlag = false;
			strSearchText = strSearchText.trim();
			String strTempXpath = "//th[@role='columnheader']/span[text()='" + strSearchBox + "']/following::input";
			...
			try
			{
				blnFlag = Page(pgeName).Element(strTempXpath).Type(strSearchText); 
				// driver.findElement(By.xpath(strTempXpath)).sendKeys(Keys.TAB);
				waitForPageRefresh();
				if(blnFlag)
				{
		...
		//''@---------------------------------------------------------------------------------------------------------------------------
		//''@Input Parameters: 
        //''@	pgeName - The Page Name where the search box is present.
        ...
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@###########################################################################################################################			
		public boolean clearSearchField(String pgeName, String strSearchBox) 
		{
			boolean blnFlag = false;	
			String strTempXpath = "//th[@role='columnheader']/span[text()='" + strSearchBox + "']/following::input";
			...
			try
			{
				blnFlag = clearFieldandWait(pgeName, strTempXpath);
				if(!blnFlag)
		...
		//''@	strInputText - Text to be typed in the input field.
		...
		//''@	strInputText - Text to be typed in the input field.
		//''@	eleButton - The button on which click action to be performed.
		//''@	pgeCommon - Page which contains the element eleError.
        ...
        //''@---------------------------------------------------------------------------------------------------------------------------
        //''@###########################################################################################################################			
		public boolean verifyErrorMessage(String pgeName, String eleInput, String strInputText, String eleButton, String pgeCommon, String eleError, String strExpText) 
		{
			boolean blnFlag = false;	
			try
			{
